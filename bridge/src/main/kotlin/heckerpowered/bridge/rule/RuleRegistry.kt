...
package heckerpowered.bridge.rule

object RuleRegistry {
    @PublishedApi
    internal val rules = mutableMapOf<Class<*>, MutableList<Any>>()
    ...
    @Suppress("UNCHECKED_CAST")
    inline fun <T : Any> forEach(type: Class<T>, action: (T) -> Unit) {
...
inline fun <reified T : Any> RuleRegistry.forEach(action: (T) -> Unit) {
